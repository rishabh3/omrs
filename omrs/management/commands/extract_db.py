...
class Command(BaseCommand):
    ...
    def export_concept(self, concept):
        ...
        extras['created'] = concept.creator
        data['date_created'] = self.datetime_handler(concept.date_created)
        # Concept Names
        names = []
        for concept_name in concept.conceptname_set.all():
            if not concept_name.voided:
                names.append({
                    ...
                    'date_created': self.datetime_handler(concept_name.date_created),
        ...
        for ref_map in concept.conceptreferencemap_set.all():
            ...
            if ref_map.concept_reference_term.concept_source.name == self.org_id:
                ...
                map_dict = self.generate_internal_mapping(
                    ...
                    term_id=ref_map.concept_reference_term.concept_reference_term_id,
                    concept_map_id=ref_map.concept_map_id,
                    creator=ref_map.creator,
                    date_created=ref_map.date_created,
                    export_concept_mapping=True)
    ...
    def export_concept_qanda(self, concept):
        ...
        for answer in concept.question_answer.all():
            map_dict = self.generate_internal_mapping(
                ...
                concept_answer_id=answer.concept_answer_id,
                export_q_and_a=True)
    ...
    def export_concept_set_members(self, concept):
        ...
        for set_member in concept.conceptset_set.all():
            map_dict = self.generate_internal_mapping(
                ...
                creator=set_member.creator,
                date_created=set_member.date_created,
                concept_set_id=set_member.concept_set_id,
                export_concept_set=True)
            maps.append(map_dict)
    ...
    def generate_internal_mapping(self, map_type=None, from_concept=None,
                                  ...
                                  retired=False, concept_map_id=None, creator=None, date_created=None, export_concept_set=False, concept_set_id=None,
                                  export_q_and_a=False, concept_answer_id=None, export_concept_mapping=False, term_id=None):
        """ Generate OCL-formatted dictionary for an internal mapping based on passed params. """
        map_dict = {}
        map_dict['map_type'] = map_type
        if export_concept_mapping:
            map_dict['concept_map_id'] = concept_map_id
            map_dict['term_id'] = term_id
        if export_concept_set:
            map_dict['concept_set_id'] = concept_set_id
        if export_q_and_a:
            map_dict['concept_answer_id'] = concept_answer_id
        map_dict['creator'] = creator
        map_dict['date_created'] = self.datetime_handler(date_created)
    ...
    def generate_external_mapping(self, map_type=None, from_concept=None,
                                  ...
                                  external_id=None, retired=False, concept_map_id=None, creator=None, date_created=None, term_id=None):
